...
fn test_tags_from_filename_compatibility() {
    ...
    for test_file in &test_files {
        ...
        let rust_tags: HashSet<String> = tags_from_filename(test_file).into_iter().map(|s| s.to_string()).collect();
...
fn test_tags_from_interpreter_compatibility() {
    ...
    for interpreter in &interpreters {
        ...
        let rust_tags: HashSet<String> = tags_from_interpreter(interpreter).into_iter().map(|s| s.to_string()).collect();
...
fn test_binary_file_compatibility() {
    ...
    temp_file.write_all(b"\x00\x01\x02\x03binary content").unwrap();
